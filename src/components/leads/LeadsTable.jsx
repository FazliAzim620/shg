...
import { leadsTableColumn } from "../constants/data";
...
const LeadsTable = ({
  ...
  const [filteredJobs, setFilteredJobs] = useState(leadsTableData?.data);
  ...
  useEffect(() => {
    const filtered = leadsTableData?.data?.filter(
      (job) =>
    ...
    setPagination({
      currentPage: leadsTableData?.current_page,
      lastPage: leadsTableData?.last_page,
      perPage: leadsTableData?.per_page,
      total: leadsTableData?.total,
    });
  }, [leadsTableData]);
            ...
            <Table stickyHeader aria-label="sticky table">
              <TableHead>
                <TableRow>
                  ...
                  {leadsTableColumn?.map((header, index) => (
                    <TableCell
                      key={index}
                      sx={{
                        ...headerStyle,
              ...
              <TableBody>
                {leadsTableData?.data?.length > 0 ? (
                  leadsTableData?.data?.map((job, index) => {
