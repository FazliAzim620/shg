...
const ClientConfirmationLetter = () => {
  ...
  const { files, uploadProgress, status, job_id } = useSelector(
  ...
  const updateItineraryCount = () => {
    dispatch(
      updateNewUserDataField({ field: "client_attachments_count", value: 1 })
    );
  };
  ...
  const deleteHandler = async () => {
    const result = await dispatch(
      deleteAttachment({ id: deleteId, type: "client" })
    );
    if (deleteAttachment.fulfilled.match(result)) {
      dispatch(deleteFile(deleteId));
      dispatch(
        updateNewUserDataField({ field: "client_attachments_count", value: 0 })
      );
      setDelteId(null);
      setShowModal(false);
    }
  ...
  const updateAttachmentStatusHandler = async () => {
    const result = await dispatch(
      updateAttachmentStatus({ id: updateId, type: "client" })
    );
          ...
          >
            ...
            {files?.length > 0 &&
              permissions?.includes(
                "create job management client confirmation letter"
              ) && (
                <Button
                  variant="contained"
                  component="label"
              ...
              {files.map((file, index) => (
                ...
                >
                  ...
                  >
                    ...
                    />
                    {permissions?.includes(
                      "delete job management client confirmation letter"
                    ) ? (
                      <CustomOutlineBtn
...
export default ClientConfirmationLetter;
