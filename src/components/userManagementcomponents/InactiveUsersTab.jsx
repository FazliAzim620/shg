...
const InactiveUsersTab = ({ currentRole }) => {
  ...
  useEffect(() => {
    const data = { role: currentRole?.name, status: 0 };
  ...
  const deleteHandler = async () => {
    try {
      ...
      if (response?.data?.success) {
        setDeleteModalOpen(false);
        dispatch(fetchUsers(currentRole?.name));
      }
    } catch (error) {
      setIsLoading(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <SpellcheckOutlinedIcon sx={{ mr: 1, fontSize: "14px" }} />
          Active
        </MenuItem>
        <MenuItem
          onClick={() => {
