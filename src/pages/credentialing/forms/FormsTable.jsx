...
import { getFormsDocument, getReferenceForms } from "../../../api_request";
...
const FormsTable = ({
  ...
  const getDocuments = async (
    ...
    try {
      ...
      const fetchedData = await getFormsDocument(data);
  ...
  const confirmDuplicate = async () => {
    try {
      ...
      const resp = await API.get(
        `/api/duplicate-cred-form/${selectedDocument?.id}`
      );
      if (resp?.data?.success) {
        setIsDuplicate(!isDuplicate);
        setIsLoading(false);
  ...
  const editHandler = () => {
    const formData = {
      ...
      has_multiple_submissions: selectedDocument?.has_multiple_submissions,
  ...
  return !isLoading &&
    ...
    >
      ...
      >
        <FormPreviewContent formData={selectedDocument} />
      </FormPreviewDrawer>
      <ConfirmStatusModal
        isOpen={isDuplicate}
        onClose={closeModal}
