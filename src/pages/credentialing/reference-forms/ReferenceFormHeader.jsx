...
import { ExpandMore, FiberManualRecordOutlined } from "@mui/icons-material";
...
const ReferenceFormHeader = () => {
  ...
  return (
    ...
    >
      ...
      {/* Multiple submissions Selection */}
      {location.state?.from === "Forms" ? (
        <>
          <Box>
            <Typography variant="body1" sx={{ my: 1 }}>
              Multiple submissions <span style={{ color: "red" }}>*</span>{" "}
            </Typography>
            <CustomSwitch
              edge="end"
              size="small"
              checked={formData.has_multi_submission}
              onChange={(event) => {
                const newValue = event.target.checked;
                const name = "has_multi_submission";
                dispatch(setRefFormData({ name, value: newValue }));
              }}
            />
          </Box>
          <Box>
            <Typography variant="body1" sx={{ my: 1 }}>
              Expires <span style={{ color: "red" }}>*</span>{" "}
            </Typography>
            <Box sx={{ display: "flex", gap: 1 }}>
              <Button
                variant="text"
                onClick={() => {
                  const name = "has_expiry";
                  const value = true;
                  dispatch(setRefFormData({ name, value }));
                }}
                sx={{
              ...
              >
                <Box
                  sx={{
                    ...
                    height: "1rem",
                    borderRadius: "100%",
                    border: formData.has_expiry
                      ? "4px solid blue"
                      : "1px solid rgba(73, 80, 87, 1)",
                  }}
                />
                Yes
              </Button>
