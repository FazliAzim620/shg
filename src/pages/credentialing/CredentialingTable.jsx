...
const CredentialingTable = ({
  ...
  const getUsers = async (currentPage = 1, perPage = 20, roleName = null) => {
    ...
    const roleNames = roleIds
      .map((roleId) => {
        const role = providerRolesList?.find((r) =>
    ...
    try {
      ...
      const fetchedData = await API.get(
        "api/admin/credentialing/get-providers",
        { params: data }
      );
      ...
      if (fetchedData?.data?.success) {
        ...
        setIsLoading(false);
        setUsers(fetchedData?.data?.data?.data);
  ...
  const viewDetailsHandler = (user, edit) => {
    navigate(`${ROUTES.credentialingDetails}`, {
      state: {
        currentUser: user,
        edit: edit || false,
      },
  ...
  const progressBar = (completed, status) => (
    <Box
      sx={{
        display: "flex",
        alignItems: "center",
    ...
    >
      <Box sx={{ width: "50%", display: "flex", alignItems: "center" }}>
        ...
        <Box
          sx={{
            bgcolor: "rgba(189, 197, 209, 1)",
            width: `${100 - parseFloat(completed)}%`,
            height: "5px",
    ...
    >
                        ...
                        return (
                          ...
                          >
                            <Typography sx={{ flexGrow: 1 }}>
                              {key.replace("_", " ")}&nbsp;:&nbsp;
                              {
                                providerRolesList?.find((r) => r.id === role)
                                  ?.name
                              }
                            </Typography>
                            <Close
                ...
                <TableRow key={user.id} hover>
                  ...
                  <TableCell>
                    <Box sx={{ display: "flex", alignItems: "center" }}>
                      ...
                      >
                        {user.name ? user?.name?.slice(0, 1) : <Person />}
                      </Avatar>
                  ...
                  <TableCell>
                    <Box sx={{ display: "flex", flexDirection: "column" }}>
                      ...
                      <Typography
                        variant="caption"
                        sx={{ mt: -0.7, textTransform: "capitalize" }}
                      >
                        {user?.specialities?.[0]?.speciality_name}
                      </Typography>
                      {/* {user?.specialities?.length > 0
                        ? user?.specialities?.map((role, index) => (
                            <Chip
                              key={index}
                              label={role?.speciality_name || "--"}
                              size="small"
                              sx={{
                  ...
                  <TableCell>
                    {progressBar(user?.compliance_requirements, user?.status)}
                  </TableCell>
      ...
      <FilterProvider
        ...
        rolesList={selectOptions(
          providerRolesList?.filter((item) => item?.name !== "provider")
        )}
...
export default CredentialingTable;
