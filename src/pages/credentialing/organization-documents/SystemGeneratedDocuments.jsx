...
import {
  getOrganizationSystemGeneratedDocuments,
  getOrganizationsDocuments,
} from "../../../api_request";
...
import { editOrganizationFormData } from "../../../feature/form-builder/organizationFormSlice";
import FormPreviewDrawer from "../dynamic-form-builder/FormPreviewDrawer";
import OrganizationPreviewForm from "./OrganizationPreviewForm";
import NodataFoundCard from "../../../provider_portal/provider_components/NodataFoundCard";

const SystemGeneratedDocuments = ({
  setSystemGeneratedDocDeleted,
  changeOccure,
  confirmDelete,
  //   viewDetailsHandler,
}) => {
  ...
  const getDocuments = async (
    ...
    try {
      ...
      const fetchedData = await getOrganizationSystemGeneratedDocuments(data);
  ...
  const viewHandler = () => {
    ...
    const formData = {
      ...
      roles: selectedDocument?.provider_roles,
      description: selectedDocument?.description,
      purpose: selectedDocument?.purpose,
    };
    dispatch(editOrganizationFormData(formData));
  };
  const editHandler = () => {
    const formData = {
      id: selectedDocument?.id,
  ...
  // <PreviewForm data={JSON.parse(preview)} />
  return !isLoading && documents?.length === 0 ? (
    ...
    >
      ...
      >
        ...
        {permissions?.includes("update user management user") && (
          <MenuItem
            onClick={() => {
              handleActionsClose();
              confirmDelete(selectedDocument, "generated");
              setSystemGeneratedDocDeleted;
            }}
            sx={{ color: "error.main" }}
          >
      ...
      <FormPreviewDrawer
        ...
        formPreview={JSON.parse(preview)}
      >
        <OrganizationPreviewForm />
      </FormPreviewDrawer>
      <ExportDrawer
...
export default SystemGeneratedDocuments;
